...
function tokenDeltasFor(tx: any, wallet: string) {
  const rows = new Map<string, { mint: string; pre: number; post: number; owner?: string; decimals: number }>();
  ...
  return Array.from(rows.values())
    .map((row) => ({ mint: row.mint, delta: Number((row.post - row.pre).toFixed(12)), decimals: row.decimals }))
...
async function explainCandidate(cfg: BotConfigRow, event: any) {
  ...
  const decision = checkEntry(cfg, event, { ...meta, isPumpFun: meta.isPumpFun || event.isPumpFun }, {
    first: !targetPrior,
    already: !!prior,
  });
  ...
  print("Candidate verdict", {
    ...
    estimatedSolSpend: Number((cfg.fixed_buy_usd / solPrice).toFixed(6)),
  });
}
...
async function main() {
  ...
  if (!signature) {
    console.log("Usage: bun run trace <target transaction signature>");
    process.exit(1);
  }
  ...
  const tx = await rpc.getParsedTransaction(signature, { commitment: "confirmed", maxSupportedTransactionVersion: 0 });
  if (!tx) throw new Error("RPC could not load that transaction. Check the signature or RPC provider.");

  const logs = (tx.meta?.logMessages ?? []).map((line) => String(line).toLowerCase());
  const swapSignal = logs.some((line) =>
  ...
  const events = decodeParsedTransaction(cfg.target_wallet, tx as any);
