export type FollowerBalance = { initial_amount: number; current_amount: number };

export function nextFollowerHop(parentHop: number): number | null {
  const normalized = Math.max(1, Math.floor(Number(parentHop) || 1));
  return normalized >= 3 ? null : normalized + 1;
}
...
export function followerSoldFraction(rows: FollowerBalance[]): number {
  ...
  const current = rows.reduce((sum, row) => sum + Math.max(0, Number(row.current_amount)), 0);
