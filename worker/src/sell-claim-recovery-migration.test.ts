...
test("prepare RPC takes the position-action lock and publishes one complete attempt", () => {
  ...
  assert.match(body, /v_position_raw := trunc\(/i);
  assert.match(
    body,
    /update public\.sell_signal_claims set[\s\S]*recovery_version = 1[\s\S]*bot_tx_sig = v_signature[\s\S]*recent_blockhash = v_blockhash[\s\S]*executed_sell_amount_raw = p_executed_sell_amount_raw[\s\S]*position_amount_before_raw = v_position_raw::text/i,
  );
  assert.match(
    body,
    /status = 'submitted' and bot_tx_sig is null and recovery_version is null/i,
  );
});
