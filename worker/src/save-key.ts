...
async function main() {
  ...
  if (!cfg)
    throw new Error("No bot_config row found. Save the target wallet in the dashboard first.");
  ...
  if (decoded.length !== 64) {
    throw new Error(
      `Private key decoded to ${decoded.length} bytes; Phantom/Solana secret keys must decode to 64 bytes.`,
    );
  }
  ...
  if (error) throw new Error(`funding_keys save failed: ${safeDiagnostic(error.message)}`);
