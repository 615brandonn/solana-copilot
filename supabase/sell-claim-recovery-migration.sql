...
begin
  ...
  v_position_raw := trunc(
    coalesce(v_position.amount_remaining, 0) * power(10::numeric, p_token_decimals)
  );
  ...
  update public.sell_signal_claims set
    ...
    position_amount_before_raw = v_position_raw::text,
    token_decimals = p_token_decimals,
    error_code = null,
    updated_at = now()
  where id = v_claim.id and user_id = p_user_id
    and status = 'submitted' and bot_tx_sig is null and recovery_version is null;
  if not found then
...
begin
  ...
  v_current_raw := trunc(coalesce(v_position.amount_remaining, 0) * v_scale);
  ...
  if v_post_raw = 0 then
    v_new_raw := 0;
  elsif v_sold_raw <= v_current_raw then
...
revoke all on function public.prepare_sell_claim_attempt_v1(
  uuid, uuid, text, text, bigint, text, text, integer
