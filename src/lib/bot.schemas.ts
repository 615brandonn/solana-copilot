...
export const BotConfigSchema = z
  .object({
    ...
    supplyAccumulationMinMarketCapUsd: z.number().finite().min(0).max(20_000),
    supplyAccumulationMaxMarketCapUsd: z.number().finite().positive().max(20_000),
    ...
    coordinatedCoinAgeMinMinutes: z.number().finite().min(0),
    coordinatedCoinAgeMaxMinutes: z.number().finite().min(0),
    ...
    coordinatedInactivityHours: z.number().finite().min(0.05),
    ...
    trailingStopPct: z.number().min(1).max(95),
    trailingActivationPct: z.number().min(1).max(100000),
